...
#z_event_endpoint <- 'itp_gen'
#z_event_endpoint <- 'myocarditis'


for (i in 1:length(endpoint_names)) {
  #i <- 1
  event.type.s  <- endpoint_names[i]
 # event.type.s  <- z_event_endpoint
  print(event.type.s)
  ...
  saveRDS(df_sccs, paste0(z_event_endpoint,".df_sccs.rds"))
  ...
  z.r <- rbind(z.agg,z.agg3,z.agg5)
  
  
 
  if(i==1) z.r4 <- z.r else
    z.r4 <- rbind(z.r4,z.r)
...
#write.csv(z.r4, "baseline numberv2.csv")


write.csv(z.r4, "baseline number.groupv2.csv")




#incidence rate cvst itp

Vaccinations <- readRDS( "Vaccinations_20211108.rds")
all_deaths <- readRDS("all_deaths_20211108.rds")
all_deaths <- subset(all_deaths, !duplicated(EAVE_LINKNO))

zz <- left_join(Vaccinations, all_deaths)

zz <- subset(zz, !is.na(date_vacc_2) & date_vacc_2 <= a.end)
zz$end <- ifelse(is.na(zz$NRS.Date.Death), as.character(a.end), as.character(zz$NRS.Date.Death))
zz$end <- as.Date(zz$end)

zz$end2 <- zz$date_vacc_2+27
zz$end2 <- ifelse(zz$end2 > zz$end , as.character(zz$end), as.character(zz$end2))
zz$end2 <- as.Date(zz$end2)

zz$pdays <- as.numeric(zz$end - zz$date_vacc_2)+1#+1 to include the vaccinated date
zz$pdays2 <- as.numeric(zz$end2 - zz$date_vacc_2)+1

no.dose <- nrow(zz)


k=1
for(event.type.s in c("any_itp","itp","CVT_SVT")){
z <- readRDS(paste(event.type.s,".df.rds",sep=""))


z <- subset(z, !is.na(z$date_vacc_2) & date_vacc_2 <= a.end)

z$end <- ifelse(is.na(z$NRS.Date.Death), as.character(a.end), as.character(z$NRS.Date.Death))
z$end <- as.Date(z$end)

z$end2 <- z$date_vacc_2+27
z$end2 <- ifelse(z$end2 > z$end , as.character(z$end), as.character(z$end2))
z$end2 <- as.Date(z$end2)

z$event <- ifelse(z$StartDate>=z$date_vacc_2 & z$StartDate <= z$end, 1, 0)
z$event2 <- ifelse(z$StartDate>=z$date_vacc_2 & z$StartDate <= z$end2, 1, 0)


z.r <- round(pois.exact(sum(z$event), no.dose/100000),2)
z.r <- rbind(z.r, round(pois.exact(sum(z$event2), no.dose/100000),2))
z.r$rateci <- paste(z.r$rate," (", z.r$lower, ",", z.r$upper,")", sep="")
z.r$event <- event.type.s
if(k==1) z.rr <- z.r else
  z.rr <- rbind(z.rr,z.r)
k=k+1
}

rate1 <- sum(z$event)/sum(zz$pdays)*10000000
rate2 <- sum(z$event2)/sum(zz$pdays2)*10000000
