...
z_df <- z_event %>% dplyr::select(EAVE_LINKNO, Age, Sex, StartDate) %>% 
  left_join(dplyr::select(Vaccinations, EAVE_LINKNO, vacc_type, date_vacc_1, date_vacc_2), by="EAVE_LINKNO") %>% 
  ...
  mutate(vt_at_event = Vacc.Type) %>% 
  mutate(vt_at_event = if_else(!is.na(Vacc.Date) &(Vacc.Date > StartDate) ,"uv", vt_at_event))
...
table(z_df$Vacc.Type, z_df$vt)

table(z_df$adm_status, z_df$dis_status, z_df$Vacc.Type)
table(z_df$adm_status, z_df$dis_status)
table(z_df$vt, z_df$days_vacc_event)
table(z_df$Vacc.Type, z_df$days_adm, exclude=NULL)

table(z_df$Vacc.Type, z_df$Vacc.Date <= z_df$StartDate, exclude=NULL)
table(z_df$Vacc.Type, z_df$AgeGrp)
table(z_df$Vacc.Type, z_df$Sex)
table(z_df$Vacc.Type, z_df$hosp_event)
